...
module spartan7_i3c_external_bus_top #(
    // System clock generated by MMCM below.  Increased from 100 MHz to
    // 140 MHz so the 2-stage SCL synchronizer in the targets eats less of
    // each I3C SDR half-period — at 140 MHz the sync latency drops from
    // ~30 ns (at 100 MHz) to ~21 ns, which makes 6 MHz I3C SDR viable on
    // real hardware.  (Hardware-validated points on this 1 kΩ pullup
    // breadboard: 100 MHz / 4 MHz works; 100 MHz / 6 MHz fails;
    // 140 MHz / 4 MHz works; 140 MHz / 6 MHz works; 140 MHz / 8 MHz fails;
    // 160 MHz / 4 MHz fails despite STA closing — VCO 640 MHz appears
    // unstable in practice on this part.)
    parameter integer CLK_FREQ_HZ = 140_000_000,
    parameter integer I3C_SDR_HZ  =  4_000_000,  // External bus rate — see build Tcl overrides for 6/8/12.5 MHz
    ...
    // ----------------------------------------------------------------
    // Clock generation — 12 MHz → 140.4 MHz via MMCM
    // VCO = 12 MHz × 58.5 = 702 MHz (within Spartan-7 600-1200 MHz range)
    // CLKOUT0 = 702 MHz / 5.0 = 140.4 MHz
    // ----------------------------------------------------------------
    wire clk_sys_unbuf;
    wire clk_sys;
    wire mmcm_locked;
